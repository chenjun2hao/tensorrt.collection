...
public:
        ...
        , mBatchSize(batchSize)
    {
        ...
        for (int i = 0; i < mEngine->getNbBindings(); i++)
        {
            ...
            manBuf->deviceBuffer = DeviceBuffer(vol, type);         // 声明 device buffer
            manBuf->hostBuffer = HostBuffer(vol, type);             // host buffer
            mDeviceBindings.emplace_back(manBuf->deviceBuffer.data());      // trt5.1 中的输入数据
            mManagedBuffers.emplace_back(std::move(manBuf));                // trt5.1 中的输入，输出数据
        }
    }
